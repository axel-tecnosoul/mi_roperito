<?php
	...
	if ( !empty($_POST)) {
		...
		$sql = "update `almacenes` set `almacen` = ?,`activo` = ? where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($_POST['almacen'],$_POST['activo'],$_GET['id']));
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT `id`, `almacen`, `activo` FROM `almacenes` WHERE id = ? ";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
