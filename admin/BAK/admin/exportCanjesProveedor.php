<?php
session_start(); 
header('Content-Disposition: attachment; filename="canjes_proveedor.xls"');
					...
					<thead>
		             ...
		             <tbody>
		              <?php 
							...
							$sql = " SELECT c.`id`, c.`fecha_hora`, a.almacen, pr.`nombre`, pr.`apellido`, c.`total` FROM `canjes` c inner join almacenes a on a.id = c.`id_almacen` inner join proveedores pr on pr.id = c.id_proveedor WHERE c.id_proveedor = ".$_SESSION['proveedor']['id'];
