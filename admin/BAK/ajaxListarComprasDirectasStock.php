...
$data_columns = $fields = ['CONCAT(pr.nombre," ",pr.apellido)','p.descripcion','a.almacen','p.precio','p.precio_costo','p.precio-p.precio_costo','p.codigo','c.categoria','s.cantidad'];

$from="FROM stock s inner join productos p on p.id = s.id_producto inner join almacenes a on a.id = s.id_almacen inner join categorias c on c.id = p.id_categoria inner join proveedores pr on pr.id = p.id_proveedor";
...
//var_dump($orderBy);
$where = "s.id_modalidad = 1 AND s.cantidad > 0 ";
...
$sql2 = "SELECT SUM(precio) AS total_precio_venta,SUM(precio_costo) AS total_precio_costo,SUM(precio)-SUM(precio_costo) AS total_ganancia $from ".($whereFiltered ? "WHERE $whereFiltered " : '');
