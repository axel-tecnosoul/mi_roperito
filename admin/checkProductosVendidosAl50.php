...
//CANJES
$sql = "SELECT cd.id AS id_canje_detalle,cd.deuda_proveedor,p.id_proveedor FROM canjes c INNER JOIN canjes_detalle cd ON cd.id_canje=c.id INNER JOIN productos p ON cd.id_producto=p.id WHERE cd.id_modalidad=50 AND cd.pagado=0 AND c.anulado=0 AND c.fecha_hora<DATE_SUB(NOW(), INTERVAL 1 MONTH)";//WHERE p.id_proveedor=667;
$q = $pdo->prepare($sql);
$q->execute();
//$ok=$ok2=0;
foreach($q->fetchAll(PDO::FETCH_ASSOC) as $row){
