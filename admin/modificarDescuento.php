...
if ( !empty($_POST)) {
  ...
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $sql = "SELECT d.id, d.descripcion, d.vigencia_desde, d.vigencia_hasta, d.minimo_compra, d.minimo_cantidad_prendas, d.monto_fijo, d.porcentaje, d.activo, GROUP_CONCAT(dfp.id_forma_pago SEPARATOR ',') as array_fdp FROM descuentos d LEFT JOIN descuentos_x_formapago dfp on id_descuento = d.id WHERE d.id = ? ";
  $q = $pdo->prepare($sql);
  $q->execute(array($id));
                ...
                <div class="card">
                  <div class="card-header">
                    <h5>Modificar Descuento</h5>
				          ...
				          <form class="form theme-form" role="form" method="post" action="modificarDescuento.php?id=<?=$id?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Detalle del Descuento</label>
                            <div class="col-sm-9"><input name="descripcion" type="text" maxlength="199" class="form-control" value="<?=$data['descripcion']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Vigencia Desde</label>
                            <div class="col-sm-9"><input name="vigencia_desde" type="date" maxlength="99" class="form-control" value="<?=$data['vigencia_desde']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Vigencia Hasta</label>
                            <div class="col-sm-9"><input name="vigencia_hasta" type="date" maxlength="99" class="form-control" value="<?=$data['vigencia_hasta']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Monto de Compra Mínimo</label>
                            <div class="col-sm-9"><input name="minimo_compra" type="number" step="0.01" maxlength="99" class="form-control" value="<?=$data['minimo_compra']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Cantidad Mínima de Ítems</label>
                            <div class="col-sm-9"><input name="minimo_cantidad_prendas" type="number" step="0" maxlength="99" class="form-control" value="<?=$data['minimo_cantidad_prendas']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row d-none">
                            <label class="col-sm-3 col-form-label">Monto Descuento Fijo</label>
                            <div class="col-sm-9"><input name="monto_fijo" type="number" step="0.01" maxlength="99" class="form-control" value="<?=$data['monto_fijo']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Porcentaje Descuento</label>
                            <div class="col-sm-9"><input name="porcentaje" type="number" step="0.1" maxlength="99" class="form-control" value="<?=$data['porcentaje']; ?>" required="required"></div>
                          </div>
                          <div class="form-group row">
