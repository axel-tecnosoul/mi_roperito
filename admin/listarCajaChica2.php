                          ...
                          <td rowspan="2" style="vertical-align: middle;" class="border-0 p-1">
                            ...
                            <select name="id_motivo" id="id_motivo" class="form-control form-control-sm filtraTabla selectpicker" data-style="multiselect" data-selected-text-format="count > 1" data-actions-box="true" data-live-search="true" multiple><?php
                              ...
                              foreach ($pdo->query($sql) as $row) {
                                $texto_anexo="";
                                if($row["id"]==22){
                                  $texto_anexo=" (Desde Mov. de caja)";?>
                                  <option value="pago_realizado" data-id-tipo-motivo="<?=$row["id_tipo_motivo"]?>"><?=$row["motivo"]?> (Desde Pagos pendientes)</option><?php
                                }?>
                                <option value="<?=$row["id"]?>" data-id-tipo-motivo="<?=$row["id_tipo_motivo"]?>"><?=$row["motivo"].$texto_anexo?></option><?php
                              }
                              Database::disconnect();?>
                            </select>
                          </td>
    ...
    <script>
      ...
      function getCaja(){
        ...
        $('#dataTables-example666').DataTable({
          ...
          ajax:{url:"listarCajaChica2GetData.php?desde="+desde+"&hasta="+hasta+"&forma_pago="+forma_pago+"&id_almacen="+id_almacen+"&id_empleado="+id_empleado+"&motivo="+motivo,dataSrc:""},
          paginate: false,
          scrollY: '100vh',
          scrollCollapse: true,
          "columns":[
