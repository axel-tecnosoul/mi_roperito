...
if ( null==$id ) {
  ...
  die("Redirecting to listarVentas.php");
}

$pdo = Database::connect();
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  
$sql = "UPDATE ventas set anulada = 1 where id = ? AND anulada = 0";
$q = $pdo->prepare($sql);
//verificar si el update se realizó correctamente para seguir con la ejecucion del codigo
if ($q->execute(array($id))) {

  $sql = " SELECT vd.id_producto, vd.cantidad, vd.subtotal, vd.id_modalidad, p.id_proveedor, v.id_almacen, vd.pagado from ventas_detalle vd inner join ventas v on v.id = vd.id_venta inner join productos p on p.id = vd.id_producto where vd.id_venta = ".$id;
