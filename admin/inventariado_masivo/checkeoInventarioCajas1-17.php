  ...
  while($param==0){//mientras el parametro siga en 0, no ha encontrado un NULL, entonces sige metiendo datos
    if($objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue()==NULL or $i==$f){//pregunta si ha encontrado un valor null en la columna
      ...
      //if($id_proveedor!="BYRO"){
        ...
        $sql2 = " SELECT p.id_proveedor,s.id_almacen,s.cantidad, inventario_ok FROM productos p LEFT JOIN stock s ON s.id_producto=p.id WHERE p.codigo = ?";
        $q2 = $pdo->prepare($sql2);
        $q2->execute([$codigo]);
        ...
        while($data = $q2->fetch(PDO::FETCH_ASSOC)){
          if($data["id_proveedor"]==$id_proveedor and $data["id_almacen"]==$id_almacen and $data["cantidad"]==$cantidad and $data["inventario_ok"]==1){
